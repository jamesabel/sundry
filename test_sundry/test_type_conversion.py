...
def test_type_conversion():
    test_values = {False: [False, 'F', 'false', 'False', 'FALSE', 'F', 'N', 'no', 0, Decimal(0), Decimal(0.0)],
                   True: [True, 'T', 'true', 'yes', 'Y', 1, Decimal(1), Decimal(1.0)],
                   None: [None, 'None', 'NULL', 'null']}
    for value, variants in test_values.items():
        for variant in variants:
            assert value == to_bool(variant)

    test_unsupported_values = {False: ['Nope', -1, 'Nada', 'fuggedaboutit', Decimal(-1)],
                               True: ['Yup', 'Si', 1.1, Decimal(100.0), Decimal(0.5)]}
